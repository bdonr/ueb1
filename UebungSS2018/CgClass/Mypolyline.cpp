...
MyPolyline::MyPolyline(unsigned int id,glm::vec3 color,std::vector<glm::vec3> x):id(id),type(Cg::Polyline){
...
void MyPolyline::LaneAlgo(){
    ...
    unsigned int neuende = this->vertices.size()-1;
...
void MyPolyline::rechne() {
    unsigned int laenge = this->vertices.size();
...
//    std::cout<<"mitte"<<mitte<<"ende"<<ende<<std::endl;
    ...
    unsigned int neuende = this->vertices.size()-1;
    for(unsigned int i =ende-2; i>=4;i--){
